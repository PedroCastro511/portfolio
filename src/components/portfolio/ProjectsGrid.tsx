...
export function ProjectsGrid() {
  return (
    <motion.div
      className="grid grid-cols-1 sm:grid-cols-2 items-stretch gap-5 md:gap-6 max-w-2xl mx-auto"
      initial="hidden"
      animate="visible"
      variants={{
      ...
      {projects.map((project) => (
        ...
        >
          <Link
            ...
            className="group relative flex h-full min-h-[280px] flex-col rounded-2xl glass shadow-card p-6 md:p-8 hover:shadow-glow hover:border-primary/40 border border-transparent transition-all duration-300"
          >
            ...
            <div className="mt-4 flex min-h-[4.5rem] w-full flex-wrap items-center justify-center gap-1.5">
              {project.languages.map((lang) => (
                <Badge
                  key={lang}
                  variant="outline"
                  className="glass shrink-0 text-[10px] md:text-xs border-border/50 pointer-events-none"
                >
                  {lang}
                </Badge>
              ))}
